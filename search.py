...
def depthFirstSearch(problem):
    ...
    while not problem.isGoalState(state):  # while we do not find the goal point
        ...
        for i in successors:
            if (not i[0] in visited) or (
                    problem.isGoalState(i[0])):  # if the successor has not been visited,push it into stack
                fringe.push((i[0], toDirection + [i[1]], toCost + i[2]))
                visited.append(i[0])  # add this point to visited list
        (state, toDirection, toCost) = fringe.pop()
...
def breadthFirstSearch(problem):
    ...
    while not problem.isGoalState(state):  # while we do not find the goal point
        successors = problem.getSuccessors(state)  # get the point's successors
        for i in successors:
            if not i[0] in visited:  # if the successor has not been visited, push it into queue
                fringe.push((i[0], toDirection + [i[1]], toCost + i[2]))
                visited.append(i[0])  # add this point to visited list
        (state, toDirection, toCost) = fringe.pop()
    ...
    while not problem.isGoalState(state):  # while we do not find the goal point
        ...
        for i in successors:
            ...
            for (visitedState, visitedToCost) in visited:
                ...
                if (i[0] == visitedState) and (total_cost >= visitedToCost):
                    visitedExist = True  # point recognized visited
                    break

            if not visitedExist:
                # push the point with priority num of its total cost
                fringe.push((i[0], toDirection + [i[1]], toCost + i[2]), toCost + i[2])
                visited.append((i[0], toCost + i[2]))  # add this point to visited list
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not problem.isGoalState(state):  # while we do not find the goal point
        ...
        for i in successors:
            ...
            if not visitedExist:
                # push the point with priority num of its total cost
                fringe.push((i[0], toDirection + [i[1]], toCost + i[2]), toCost + i[2] + heuristic(i[0], problem))
                visited.append((i[0], toCost + i[2]))  # add this point to visited list
